...
# point styles - enhanced visibility
PREV_SIZE   = 15             # size for previous points (increased)
PREV_ALPHA  = 0.70           # alpha for previous points (increased)
CURR_SIZE   = 80             # size for current point (increased)
CURR_ALPHA  = 1.00           # alpha for current point

# edge styles - more vibrant
EDGE_COLOR  = 'cyan'         # color for connecting lines (changed to cyan)
EDGE_ALPHA  = 0.80           # alpha for connecting lines (increased)
EDGE_WIDTH  = 2.0            # width for connecting lines (increased)
...
def create_cumulative_fixation_image_with_edges_v1(data_up_to_row, current_row, output_path,
    ...
    # Plot previous points (all but current) - vectorized scatter with enhanced visibility
    if len(x_coords) > 1:
        prev_x = x_coords[:-1]
        prev_y = y_coords[:-1]
        ax.scatter(prev_x, prev_y, s=prev_size, c='orange', alpha=prev_alpha,
                  edgecolors='white', linewidths=1.0, zorder=3)

    # Plot the current fixation point with bright highlight (highest z-order to avoid overlap)
    ax.scatter([x_coords[-1]], [y_coords[-1]], s=curr_size, c='yellow', alpha=curr_alpha,
...
N = len(df)
print(f"Generating {N} images with enhanced visibility (v2)...")
...
print("Done with v2 enhanced visibility.")
